...
wx.ready(function () {
  ...
  document.querySelector('#chooseImage').onclick = function () {
    wx.chooseImage({
      ...
      success: function (res) {
        images.localId = res.localIds;      //照片的本地Id（多个）

        // $('#image-id img').attr('src',images.localId);
        alert(res.localIds);
  ...
  document.querySelector('#previewImage').onclick = function () {
    wx.previewImage({
      current: 'http://miyacloud.cn/apily/Public/Home/img/1.png',       //当前图片
      urls: [
        'http://miyacloud.cn/apily/Public/Home/img/1.png',              //待查看图片列表
        'http://miyacloud.cn/apily/Public/Home/img/2.png',
      ]
    });
  };

  // 5.3 上传图片  传图片有效期3天，可用微信多媒体接口下载图片到自己的服务器，此处获得的 serverId 即 media_id。
  ...
  // 7.3 使用微信内置地图查看当前地理位置接口
  document.querySelector('#openNowLocation').onclick = function () {
    wx.getLocation({
      success: function (res) { 
        wx.openLocation({
